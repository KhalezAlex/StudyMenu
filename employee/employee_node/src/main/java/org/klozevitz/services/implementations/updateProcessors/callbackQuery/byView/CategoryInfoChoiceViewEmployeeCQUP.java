...
public class CategoryInfoChoiceViewEmployeeCQUP implements UpdateProcessor {
    ...
    public ArrayList<SendMessage> processUpdate(Update update) {
        ...
        } else if (command.contains("/category_info_")) {
            return categoryInfoViewResolver.processUpdate(update);
        } else {
            return previousViewUpdateProcessor.processUpdate(update);
        }
    }
