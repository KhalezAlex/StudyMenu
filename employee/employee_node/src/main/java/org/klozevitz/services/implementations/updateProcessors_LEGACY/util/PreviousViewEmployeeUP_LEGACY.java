...
public class PreviousViewEmployeeUP_LEGACY implements UpdateProcessor_LEGACY<Update, EmployeeView> {
    ...
    public SendMessage processUpdate(Update update, EmployeeView employeeView) {
        var tgUserId = tgUserId(update);
        var currentAppUser = appUserRepo.findByTelegramUserId(tgUserId).get();
        var previousView = previousView(currentAppUser);
    ...
    private long tgUserId(Update update) {
