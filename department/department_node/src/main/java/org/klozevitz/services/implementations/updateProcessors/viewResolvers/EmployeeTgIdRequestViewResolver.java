...
import static org.klozevitz.enitites.appUsers.enums.states.DepartmentState.WAIT_FOR_EMPLOYEE_TG_ID_STATE;
import static org.klozevitz.enitites.appUsers.enums.views.DepartmentView.EMPLOYEE_TG_ID_REQUEST_VIEW;

@Log4j
@RequiredArgsConstructor
public class EmployeeTgIdRequestViewResolver extends BasicUpdateProcessor {
    ...
    public ArrayList<SendMessage> processUpdate(Update update) {
        ...
        departmentRepo.setDepartmentCurrentView(EMPLOYEE_TG_ID_REQUEST_VIEW.name(), telegramUserId);
        departmentRepo.setDepartmentState(WAIT_FOR_EMPLOYEE_TG_ID_STATE.name(), telegramUserId);

        var answer = telegramView.employeeTgIdRequestView(update);
